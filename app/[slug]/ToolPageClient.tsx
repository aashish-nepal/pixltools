...
export default function ToolPageClient({ tool }: Props) {
    ...
    const [aspectRatio, setAspectRatio] = useState("16:9"); // #14 fix
    ...
    // #3 fix: update crop defaults when a new image is loaded
    const handleImageLoad = useCallback((e: React.SyntheticEvent<HTMLImageElement>) => {
        if (tool.slug === "crop-image") {
            const img = e.currentTarget;
            setCropWidth(img.naturalWidth || 400);
            setCropHeight(img.naturalHeight || 300);
        }
    }, [tool.slug]);
    ...
    const getExtraParams = (): Record<string, string | number> => {
        ...
        if (s === "rotate-image") return { angle: rotateAngle };
        if (s === "flip-image") return { direction: flipDir };
        // #14 fix: aspect-ratio uses its own preset dimensions, not the raw resize inputs
        if (s === "image-aspect-ratio") return { width: aspectPresets[aspectRatio].w, height: aspectPresets[aspectRatio].h };
        if (s.includes("resize") || s === "reduce-image-file-size")
    ...
    const renderControls = (onProcess: () => void, loading: boolean) => {
        ...
        return (
            <div className="space-y-4">
                {/* Per-tool controls */}
                {s === "resize-image" || s === "reduce-image-file-size" ? (
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="text-sm text-slate-400 mb-1 block">Width (px)</label>
                            <input type="number" value={resizeW} onChange={e => setResizeW(+e.target.value)} className="w-full border border-slate-700 bg-slate-800 text-slate-200 rounded-lg px-3 py-2 text-sm" />
                        </div>
                        <div>
                            <label className="text-sm text-slate-400 mb-1 block">Height (px)</label>
                            <input type="number" value={resizeH} onChange={e => setResizeH(+e.target.value)} className="w-full border border-slate-700 bg-slate-800 text-slate-200 rounded-lg px-3 py-2 text-sm" />
                        </div>
                    </div>
                ) : null}

                {/* #14 fix: aspect-ratio tool shows preset ratio buttons, not raw pixel inputs */}
                {s === "image-aspect-ratio" ? (
                    <div>
                        <label className="text-sm text-slate-400 mb-2 block">Target Aspect Ratio</label>
                        <div className="flex gap-2 flex-wrap">
                            {Object.entries(aspectPresets).map(([ratio, dims]) => (
                                <button
                                    key={ratio}
                                    onClick={() => setAspectRatio(ratio)}
                                    className={`px-4 py-2 rounded-lg text-sm font-medium border transition-all ${aspectRatio === ratio ? "bg-emerald-500 text-slate-950 border-emerald-500" : "border-slate-700 text-slate-400 hover:border-emerald-500/50"}`}
                                >
                                    {ratio}
                                    <span className="ml-1.5 text-xs opacity-60">{dims.w}×{dims.h}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                {s === "crop-image" ? (
                    <div className="grid grid-cols-2 gap-3">
                        {[["Left", cropLeft, setCropLeft], ["Top", cropTop, setCropTop], ["Width", cropWidth, setCropWidth], ["Height", cropHeight, setCropHeight]].map(([label, val, setter]) => (
                            <div key={String(label)}>
                                <label className="text-sm text-slate-400 mb-1 block">{String(label)} (px)</label>
                                <input type="number" value={val as number} onChange={e => (setter as (v: number) => void)(+e.target.value)} className="w-full border border-slate-700 bg-slate-800 text-slate-200 rounded-lg px-3 py-2 text-sm" />
                            </div>
                        ))}
                    </div>
                ) : null}
                ...
                {s === "rotate-image" ? (
                    <div>
                        <label className="text-sm text-slate-400 mb-1 block">Rotation Angle</label>
                        <div className="flex gap-2 flex-wrap">
                            {[90, 180, 270].map(a => (
                                <button key={a} onClick={() => setRotateAngle(a)} className={`px-4 py-2 rounded-lg text-sm font-medium border ${rotateAngle === a ? "bg-emerald-500 text-slate-950 border-emerald-500" : "border-slate-700 text-slate-400 hover:border-emerald-500/50"}`}>{a}°</button>
                            ))}
                            <input type="number" value={rotateAngle} onChange={e => setRotateAngle(+e.target.value)} className="border border-slate-700 bg-slate-800 text-slate-200 rounded-lg px-3 py-2 text-sm w-24" placeholder="Custom" />
                        </div>
                    </div>
                ) : null}
                ...
                {s === "flip-image" ? (
                    <div>
                        <label className="text-sm text-slate-400 mb-1 block">Flip Direction</label>
                        <div className="flex gap-2">
                            {(["horizontal", "vertical"] as const).map(d => (
                                <button key={d} onClick={() => setFlipDir(d)} className={`px-4 py-2 rounded-lg text-sm font-medium border ${flipDir === d ? "bg-emerald-500 text-slate-950 border-emerald-500" : "border-slate-700 text-slate-400 hover:border-emerald-500/50"}`}>
                ...
                {s === "blur-image" ? (
                    <div>
                        <label className="text-sm text-slate-400 mb-1 block">Blur Strength: {blurSigma}</label>
                        <input type="range" min={1} max={20} value={blurSigma} onChange={e => setBlurSigma(+e.target.value)} className="w-full accent-emerald-500" />
                    </div>
                ) : null}

                {s === "image-brightness" ? (
                    <div>
                        <label className="text-sm text-slate-400 mb-1 block">Brightness: {brightness.toFixed(1)}</label>
                        <input type="range" min={0.1} max={3} step={0.1} value={brightness} onChange={e => setBrightness(+e.target.value)} className="w-full accent-emerald-500" />
                    </div>
                ) : null}

                {s === "image-contrast" ? (
                    <div>
                        <label className="text-sm text-slate-400 mb-1 block">Contrast: {contrast.toFixed(1)}</label>
                        <input type="range" min={0.1} max={3} step={0.1} value={contrast} onChange={e => setContrast(+e.target.value)} className="w-full accent-emerald-500" />
                    </div>
                ) : null}

                {s === "watermark-image" ? (
                    <div>
                        <label className="text-sm text-slate-400 mb-1 block">Watermark Text</label>
                        <input type="text" value={watermarkText} onChange={e => setWatermarkText(e.target.value)} className="w-full border border-slate-700 bg-slate-800 text-slate-200 rounded-lg px-3 py-2 text-sm" placeholder="© Your Name" />
                    </div>
                ) : null}
                ...
                {s === "image-border" ? (
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="text-sm text-slate-400 mb-1 block">Border Width (px): {borderWidth}</label>
                            <input type="range" min={1} max={100} value={borderWidth} onChange={e => setBorderWidth(+e.target.value)} className="w-full accent-emerald-500" />
                        </div>
                        <div>
                            <label className="text-sm text-slate-400 mb-1 block">Border Color</label>
                            <input type="color" value={borderColor} onChange={e => setBorderColor(e.target.value)} className="w-full h-10 rounded-lg border border-slate-700 cursor-pointer" />
                        </div>
                    </div>
                ) : null}
                ...
                {s === "image-upscaler" ? (
                    <div>
                        <label className="text-sm text-slate-400 mb-1 block">Upscale Factor</label>
                        <div className="flex gap-2">
                            {[2, 3, 4].map(x => (
                                <button key={x} onClick={() => setUpscale(x)} className={`px-4 py-2 rounded-lg text-sm font-medium border ${upscale === x ? "bg-emerald-500 text-slate-950 border-emerald-500" : "border-slate-700 text-slate-400 hover:border-emerald-500/50"}`}>{x}×</button>
                            ))}
                        </div>
                    </div>
                ) : null}
                ...
                {s === "image-thumbnail" ? (
                    <div>
                        <label className="text-sm text-slate-400 mb-1 block">Thumbnail Preset</label>
                        <div className="flex gap-2 flex-wrap">
                            {Object.entries(thumbPresets).map(([key, dims]) => (
                                <button key={key} onClick={() => setThumbPreset(key)} className={`px-3 py-2 rounded-lg text-sm font-medium border ${thumbPreset === key ? "bg-emerald-500 text-slate-950 border-emerald-500" : "border-slate-700 text-slate-400 hover:border-emerald-500/50"}`}>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                    <button
                        onClick={onProcess}
                        disabled={loading}
                        className="flex-1 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-60 text-slate-950 font-bold py-3 px-6 rounded-xl transition-colors flex items-center justify-center gap-2"
                    >
                        {loading ? (
    ...
    const isJsonTool = tool.slug === "image-color-picker" || tool.slug === "image-metadata-viewer";
    ...
    const renderJsonResult = (data: Record<string, unknown>) => {
        if (tool.slug === "image-color-picker") {
            ...
            return (
                <div>
                    <h3 className="font-bold text-slate-300 mb-3 text-sm">Dominant Color Palette</h3>
                    <div className="flex flex-wrap gap-2 mb-4">
                        {palette.map((c, i) => (
                            <div key={i} className="text-center">
                                <div
                                    className="w-12 h-12 rounded-xl border border-slate-700 shadow-sm cursor-pointer hover:scale-110 transition-transform relative"
                                    style={{ background: c.hex }}
                                    onClick={() => handleCopyHex(c.hex)}
                                    title={`Click to copy ${c.hex}`}
                                >
                                    {copiedHex === c.hex && (
                                        <span className="absolute -top-7 left-1/2 -translate-x-1/2 bg-slate-700 text-emerald-400 text-xs font-bold px-2 py-0.5 rounded-md whitespace-nowrap">
                                            Copied!
                                        </span>
                                    )}
                                </div>
                                <p className="text-xs text-slate-500 mt-1 font-mono">{c.hex}</p>
                            </div>
                        ))}
                    </div>
                    <h3 className="font-bold text-slate-300 mb-2 text-sm">Channel Statistics</h3>
                    <div className="grid grid-cols-2 gap-2">
                        {channels.map((ch, i) => (
                            <div key={i} className="bg-slate-900 rounded-lg p-2 text-xs">
                                <span className="font-semibold text-slate-300">{ch.channel}</span>
                                <span className="text-slate-500 ml-2">Mean: {ch.mean} · Min: {ch.min} · Max: {ch.max}</span>
                            </div>
                        ))}
                    </div>
                </div>
        ...
        if (tool.slug === "image-metadata-viewer") {
            ...
            return (
                <div>
                    <h3 className="font-bold text-slate-300 mb-3 text-sm">Image Metadata</h3>
                    <div className="overflow-x-auto">
                        <table className="w-full text-xs">
                            <tbody>
                                {entries.map(([k, v]) => (
                                    <tr key={k} className="border-b border-slate-700 last:border-none">
                                        <td className="py-1.5 pr-4 font-mono font-semibold text-emerald-400 w-1/3">{k}</td>
                                        <td className="py-1.5 text-slate-500 break-all">{String(v)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
